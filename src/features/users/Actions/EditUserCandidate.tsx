...
import { useUpdateCandidateMutation } from "@/lib/api/applicationApi";
import { useUpdateUserCandidateMutation } from "@/lib/api/userApi";
import NumberInput from "@/components/Form/NumberInput";

export default function EditUserCandidate({ user }: { user: any }) {
  const [updateUserCandidate, { error, isLoading }] =
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      ...
      // console.log(values);
      try {
        const response = await updateUserCandidate({
          data: { status: values.status, schoolFees: values.schoolFees },
          id: user._id,
        }).unwrap();
