...
} from "@/lib/api/coursApi";

export default function EditCours({ user }: { user: any }) {
  const [updateCours, { error, isLoading }] = useUpdateCoursMutation();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      ...
      setSubmitting(true);
      try {
        const response = await updateCours({
          data: data,
          id: user._id,
        }).unwrap();
  ...
  return (
    <>
      ...
      <Modal isOpen={open} title="Modification cours" onClose={handleClose}>
